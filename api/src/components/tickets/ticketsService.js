...
const getTicket = async(data) => {
    try {
        const response = await Ticket.findOne({
            ...
            if (!ticket) {
                //elasticsearch.run('getticket.unexisted', `Ce ticket n'existe pas. ${data.number}`, 404)
                return { status: 404, data: { message: "Ce ticket n'existe pas.", successful: false } };
            }
            else if(ticket.userId != null){
                //elasticsearch.run('getticket.existed', `Ce ticket a déjà été validé. ${data.number}`, 401)
                ...
                return {status: 200, data : {ticket, successful: true}};
            }
        });
        return response;
    } catch (e) {
...
const updateTicket = async(userId, data) => {
    try {
        ...
        //elasticsearch.run('updateticket.success', `Une erreur inattendue est survenue lors de la tentative de mise à jour du ticket. Veuillez réessayer. ${e.message}`, 400)
        return {
            status: 400,
            data: {
...
const printTicket = async () => {
    try {
        const response = await Ticket.findOne({
            ...
            //elasticsearch.run('printticket.success', `Iicket ${ticket} imprimé.`, 200)
            return {
                status: 200,
                data: {
                    ticket: ticket,
