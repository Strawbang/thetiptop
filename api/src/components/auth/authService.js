...
const signup = async (data) => {
    try {
        const response = await User.create({
            ...
            const confEmail = await mailService.sendConfirmationEmail(user, token);
            ...
            //elasticsearch.run('signup.successfull', `Le compte ${user.id} à bien été créé et le mail de confirmation a était envoyé`, 200)
            return {
                status: 200,
                data: {
...
const signin = async (data) => {
    try {
        ...
        const response = await User.findOne({ where: { email }
            ...
            if (!user){
                //elasticsearch.run('signin.failedusername', `Le nom d'utilisateur que vous avez entré est invalide. ${user.email}`, 400)
                return {
                    status: 404,
                    data: {
        ...
        //elasticsearch.run('signin.error', `Une erreur inattendue est survenue. Veuillez réessayer. ${user.email}`)
        return { status: 500, data: { message: "Une erreur inattendue est survenue. Veuillez réessayer.", error: e.message } };
    }
}
...
const resetPassword = async (email) => {
    try {
        User.findOne(
            ...
            // await emailService.sendEmailPassword(user.email, user.firstname, password);
        });

        return { status: 200, data: { successful: true, message: 'Votre mot de passe viens de vous être envoyé par email' } };
    } catch (e) {
        return { status: 500, data: { successful: false, message: 'Une erreur s\'est produite lors de la réinitilisation de votre mot de passe' } };
    }
};
